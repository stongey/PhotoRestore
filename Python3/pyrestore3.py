...
def my_palette(small):
    ...
    colours.sort(key=functools.cmp_to_key(lambda C1, C2: cmp(Y(C1[0], C1[1], C1[2]), Y(C2[0], C2[1], C2[2]))))
...
def first_restore():
    ...
    if not (ok_r and ok_g and ok_b):
        pdb.gimp_message("The program has failed to obtain a satisfactory restoration; the result shown may be poor.")
...
def restore(im):
    ...
    # Convert restoration parameters Lambda1 and Sigma1 to parameters for gimp levels command.
    (alpha1, m1, s1) = levels_params(Lambda1, Sigma1, C_hi)

    # Restore the small image using replacement for gimp levels command and get its colourmap.
...
for param in sys.argv:
    Dir = param.find("dir=")
    if Dir >= 0:
        directory = param[Dir+4:]
    gam = param.find("Light-Dark=")
    if gam >= 0:
        gammatarg = float(param[gam+11:])
    sat = param.find("saturate=")
    if sat >= 0:
        sat_choice = eval(param[sat+9:])
...
# Make look up table for conversion of RGB to LUV, needs to be bigger than 256 in case search explores here.
